...
function buildSystemPrompt(input: AiReplyInput): string {
  ...
  return [
    'شما دستیار پاسخ‌گوی دایرکت اینستاگرام برای یک کسب‌وکار هستید.',
    'بر اساس «دانش» زیر و فقط با اتکا به آن، یک پاسخ کوتاه و دوستانه برای',
    'کامنت کاربر بنویس که به‌صورت پیام مستقیم (دایرکت) ارسال می‌شود.',
